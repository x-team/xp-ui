...
class InputField extends PureComponent<Props> {
  ...
  renderField = () => {
    ...
    return Tag({
      ...baseProps,
      className: `${errorClassName} ${spacingClassName} ${type === 'date' ? dateInput : ''}`,
      type,
      ...rest
    })
  }
  ...
  render () {
    ...
    const dateInputClassContainer = type === 'date' ? dateInputContainer : ''
          ...
          ? (
            <label id={labelId}>
              ...
              {dateInputClassContainer !== '' ? (
                <div className={dateInputClassContainer}>
                  {this.renderField()}
                </div>
              ) : this.renderField()}
              {postLabel}
